...
std::optional<slam::LoopResult> LoopClosure::detect(const cv::Mat& descriptors,
                                                    const std::vector<cv::KeyPoint>& keypoints,
    ...
    if (!(best_candidate.score > m_params.min_absolute_score && is_significant_enough)) {
        SPDLOG_DEBUG("Best candidate (ID {}) score {} not significant enough.", best_candidate.id,
    ...
    cv::Mat rvec, tvec;
    cv::Mat inliers;
    cv::solvePnPRansac(points_3d, points_2d, camera.K, camera.D, rvec, tvec, false, 100, 4.0, 0.99,
                       inliers);
    ...
    if (inliers.rows >= m_params.min_inliers) {
        ...
        SPDLOG_WARN("Geometric verification FAILED: Only {} inliers found (minimum required: {}).",
                    inliers.rows, m_params.min_inliers);
    }
