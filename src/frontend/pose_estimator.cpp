...
void PoseEstimator::estimate(const std::vector<KeyDescriptorPair>& pairs1,
    ...
    if (matches.size() < 8) {
        SPDLOG_WARN("Cannot estimate pose, not enough matches ({}). Required at least 8.", matches.size());
        return;
    }

    // 1. Convert the matches into vector<Point2f> as required by OpenCV's epipolar geometry functions.
...
std::vector<cv::Point3d> PoseEstimator::triangulatePoints(const std::vector<cv::KeyPoint>& keypoints1,
